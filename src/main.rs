...
static PROJECT_ID: &str = "GCP PROJECT ID";
static LOCATION: &str = "LOCATION";
static REGISTRY_ID: &str = "YOUR REGISTRY ID";
static DEVICE_ID: &str = "YOUR DEVICE ID";
...
fn main() {
    ...
    // println!("key: {}", key);
    let token = jwt::encode(&header, &claims, &EncodingKey::from_rsa_der(include_bytes!("../rsa_private.der"))).unwrap();
